        ...
        <div id="hist-card-${idx}" class="bg-white p-5 rounded-[2.5rem] border border-slate-100 mb-3 shadow-sm transition-all overflow-hidden" onclick="toggleOrderExpand(${idx})">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button onclick="event.stopPropagation(); reorder(${idx})" class="p-2 bg-slate-50 rounded-xl text-slate-400 hover:text-blue-600 transition-colors"><i data-lucide="refresh-cw" class="w-4 h-4"></i></button>
                </div>
            </div>
        ...
        <div class="bg-white p-4 rounded-3xl border border-slate-100 flex items-center gap-4">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-center gap-2 mt-1">
                    <input type="text" value="${p.cat || 'Other'}" onchange="editItem(${p.id}, 'cat', this.value)" placeholder="Category" class="text-[9px] font-black uppercase text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md outline-none">
...
window.importDatabase = (e) => {
    ...
    reader.onload = (event) => { 
        try { 
            ...
            closeBackupModal(); 
        } catch { alert("Error: Invalid JSON File"); }
    };
...
window.reorder = idx => { const item = history[idx]; orderCounter++; queue.unshift({ orderNum: orderCounter, items: JSON.parse(JSON.stringify(item.items)), total: item.total, desc: item.desc || "", date: new Date().toLocaleTimeString() }); pushData(); };
...
window.removeItem = id => { if(confirm("Remove this item?")) { products = products.filter(x => x.id !== id); pushData(); } };
...
// --- RESET TRIGGER ---
function startReset() { resetTimer = setTimeout(() => { if (confirm("Wipe all data and reset system?")) { db.ref('/').set({ products: [], queue: [], history: [], orderCounter: 0 }); location.reload(); } }, 1000); }
