...
let selectedIds = new Set(); // Stores IDs for bulk delete
...
    // Bulk delete UI update
    const bulkBar = document.getElementById('bulk-bar');
    if(bulkBar) {
        bulkBar.classList.toggle('hidden', selectedIds.size === 0);
        document.getElementById('selected-count').innerText = `${selectedIds.size} Items Selected`;
    }

    const inventoryList = document.getElementById('inventory-list');
    if(inventoryList) {
        inventoryList.innerHTML = products.map((p, idx) => `
            <div class="bg-white p-5 rounded-[2.5rem] border ${selectedIds.has(p.id) ? 'border-blue-500 bg-blue-50/30' : 'border-slate-100'} space-y-4 shadow-sm">
                <div class="flex items-center gap-4">
                    <input type="checkbox" ${selectedIds.has(p.id) ? 'checked' : ''} onchange="toggleSelectProduct(${p.id})" class="bulk-checkbox">
...
            const [name, price, img, cat] = row.split(',').map(item => item.trim());
            return { id: Date.now() + Math.random(), name: name || 'New Product', price: parseFloat(price) || 0, img: img || '', fav: false, cat: cat || '' };
        });
...
function renderPendingAndHistory() {
    ...
    if(hList) {
        hList.innerHTML = `<h2 class="font-black text-lg px-2 mb-4 text-slate-400">History</h2>` + history.map((h, idx) => `
            <div id="hist-card-${idx}" class="bg-white p-5 rounded-[2.5rem] border border-slate-100 mb-3 cursor-pointer transition-all overflow-hidden" onclick="toggleOrderExpand(${idx})">
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        <span class="text-slate-400 font-black text-[10px]">#${h.orderNum}</span>
                        <input type="text" value="${h.desc || ''}" 
                            onclick="event.stopPropagation()" 
                            onchange="updateTag('history', ${idx}, this.value)" 
                            placeholder="Customer Name..."
                            class="font-bold text-slate-700 text-sm bg-transparent outline-none">
                    </div>
                    <div class="flex items-center gap-3">
                        <p class="font-black text-blue-600 text-sm">$${h.total.toFixed(2)}</p>
                        <button onclick="event.stopPropagation(); reorder(${idx})" class="p-2 bg-slate-50 rounded-xl text-slate-400 hover:text-blue-600"><i data-lucide="refresh-cw" class="w-4 h-4"></i></button>
                    </div>
                </div>
                
                <div id="details-${idx}" class="hidden mt-4 pt-4 border-t border-slate-50">
                    <div class="space-y-2 mb-4">
                        ${h.items.map(i => `
                            <div class="flex justify-between items-center">
                                <span class="text-[11px] font-bold text-slate-600">${i.name} <span class="text-slate-400">x${i.qty}</span></span>
                                <span class="text-[10px] font-black text-slate-400">$${(i.price * i.qty).toFixed(2)}</span>
                            </div>
                        `).join('')}
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-[10px] text-slate-300 font-bold uppercase">${h.date || ''}</span>
                        <button onclick="event.stopPropagation(); removeItemFromList('history', ${idx})" class="text-red-400 p-2 hover:bg-red-50 rounded-xl transition-colors">
...
window.toggleOrderExpand = (idx) => {
    ...
    if(el) {
        ...
        // Close all others first (optional, for accordion style)
        document.querySelectorAll('[id^="details-"]').forEach(d => d.classList.add('hidden'));
        document.querySelectorAll('[id^="hist-card-"]').forEach(c => c.classList.remove('border-blue-200', 'shadow-md'));
