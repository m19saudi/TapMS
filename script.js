        ...
        inventoryList.innerHTML = products.map((p, idx) => `
            <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 space-y-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <input type="text" value="${p.name}" onchange="editItem(${p.id}, 'name', this.value)" class="w-full font-extrabold text-sm bg-transparent outline-none truncate" placeholder="Item Name">
                ...
                <div class="bg-slate-50 rounded-[1.5rem] p-3 border border-slate-100">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <input type="text" value="${p.cat || ''}" onchange="editItem(${p.id}, 'cat', this.value)" placeholder="Category name..." class="flex-1 bg-white px-3 py-1.5 rounded-lg text-[11px] font-black text-blue-600 border border-slate-200 outline-none uppercase">
                    </div>
...
window.confirmWipeCart = () => { if(confirm("Clear current cart?")) { cart = []; closeCart(); render(); } };
...
function renderPendingAndHistory() {
    ...
    if(pList) {
        pList.innerHTML = `<h2 class="font-black text-lg px-2 mb-4">Pending Orders</h2>` + queue.map((ord, idx) => `
            <div class="bg-blue-50/50 p-5 rounded-[2.5rem] border-2 border-blue-100">
                <div class="bg-white px-3 py-2 rounded-xl border border-blue-100 flex items-center gap-2 mb-3">
                    ...
                    <input type="text" value="${ord.desc || ''}" onchange="updateTag('queue', ${idx}, this.value)" placeholder="Customer tag..." class="bg-transparent font-bold text-blue-600 text-sm outline-none w-full">
                </div>
                <div class="flex flex-wrap gap-2 mb-4">
                    ${ord.items.map(i => `<span class="bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-1 rounded-lg border border-blue-200">${i.name} x${i.qty}</span>`).join('')}
                </div>
    ...
    if(hList) {
        hList.innerHTML = `<h2 class="font-black text-lg px-2 mb-4 text-slate-400">Past Orders</h2>` + history.map((h, idx) => `
            <div id="hist-card-${idx}" class="bg-white p-5 rounded-[2.5rem] border border-slate-100 mb-3" onclick="this.classList.toggle('order-expanded')">
                ...
                <div class="order-detail">
                    <p class="text-[9px] text-slate-400 font-bold mb-3 uppercase">Purchased at ${h.date}</p>
                    <div class="flex flex-wrap gap-2 mb-4">
                    ...
                    <div class="flex gap-2">
                        <button onclick="event.stopPropagation(); reorder(${idx})" class="flex-1 bg-slate-900 text-white py-3 rounded-xl font-black uppercase text-[10px]">Re-Order</button>
                        <button onclick="event.stopPropagation(); removeItemFromList('history', ${idx})" class="px-4 bg-red-50 text-red-400 rounded-xl"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                    </div>
                </div>
            </div>`).join('');
    }
...
window.resetOnlyOrders = () => { if(confirm("Wipe all order history?")) { queue = []; history = []; orderCounter = 0; pushData(); closeBackupModal(); } };
