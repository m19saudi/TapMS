...
function render() {
    ...
    document.getElementById('pending-list').innerHTML = `<h2 class="font-black text-lg px-2 mb-4">Pending Review</h2>` + queue.map((ord, idx) => `
        <div class="bg-blue-50/50 p-5 rounded-[2.5rem] border-2 border-blue-100">
            <div class="bg-white px-3 py-2 rounded-xl border border-blue-100 flex items-center gap-2 mb-3">
                ...
                <input type="text" value="${ord.desc || ''}" onchange="updateTag('queue', ${idx}, this.value)" placeholder="Order Note..." class="bg-transparent font-bold text-blue-600 text-sm outline-none w-full">
            </div>
    ...
    document.getElementById('history-list').innerHTML = `<h2 class="font-black text-lg px-2 mb-4 text-slate-400">History</h2>` + history.map((h, idx) => `
        <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 mb-4 shadow-sm">
            <div class="flex justify-between items-start mb-3">
                <div>
                    <div class="flex items-center gap-2">
                         ...
                         <input type="text" value="${h.desc || ''}" onchange="updateTag('history', ${idx}, this.value)" class="font-bold text-slate-700 text-sm bg-transparent outline-none">
                    </div>
                    <p class="text-[10px] text-slate-400 font-bold mt-1">${h.date}</p>
                </div>
            ...
            <div class="flex gap-2">
                <button onclick="reorder(${idx})" class="flex-1 bg-slate-900 text-white py-3 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2">
                    <i data-lucide="refresh-cw" class="w-3 h-3 text-slate-400"></i> Reorder
                </button>
                <button onclick="removeItemFromList('history', ${idx})" class="px-4 py-3 bg-red-50 text-red-400 rounded-xl"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
            </div>
        </div>
    `).join('') || '<p class="text-center py-10 opacity-20 font-bold uppercase text-[10px]">Empty History</p>';
    ...
    document.getElementById('inventory-list').innerHTML = products.map(p => `
        <div class="bg-white p-4 rounded-3xl border border-slate-100 flex items-center gap-4">
            <div class="relative w-14 h-14 shrink-0 overflow-hidden rounded-2xl bg-slate-50">
                ...
                <input type="text" value="${p.img || ''}" onchange="editItem(${p.id}, 'img', this.value)" placeholder="URL" class="absolute inset-0 opacity-0 focus:opacity-100 bg-white/95 text-[8px] text-center font-bold outline-none">
            </div>
            <div class="flex-1">
            ...
            <div class="flex flex-col gap-2">
                ...
                <button onclick="removeItem(${p.id})" class="text-red-100 hover:text-red-400 transition-all"><i data-lucide="trash-2" class="w-5 h-5"></i></button>
            </div>
        </div>
    `).join('');
...
window.toggleManageSection = sec => {
    ...
    const btnOrders = document.getElementById('sub-btn-orders');
    const btnStock = document.getElementById('sub-btn-stock');
    
    if(sec === 'orders') {
        btnOrders.className = "px-6 py-2 rounded-xl text-xs font-black uppercase bg-white shadow-sm text-blue-600";
        btnStock.className = "px-6 py-2 rounded-xl text-xs font-black uppercase text-slate-400";
    } else {
        btnStock.className = "px-6 py-2 rounded-xl text-xs font-black uppercase bg-white shadow-sm text-blue-600";
        btnOrders.className = "px-6 py-2 rounded-xl text-xs font-black uppercase text-slate-400";
    }
};
